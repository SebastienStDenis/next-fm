...
from cli.geonames import CityRecord, fetch_cities


async def seed_cities(session: AsyncSession, cities: list[CityRecord]) -> None:
    stmt = pg_insert(City)
...
async def seed() -> None:
    ...
    async with session_factory() as session:
        await seed_cities(session, cities)
